...
\DeclareFactoid{Shape}{%
\begin{tabular}{ccc}
  \textbf{Shape} & \textbf{Area} & \textbf{Perimeter} \\
  % circle
  \tikz [baseline={(d.base)}] {
    \useasboundingbox (-1.2, -1.2) rectangle (1.2, 1.2);
    \draw circle (1);
    \draw [dashed] (0, -1) -- node [left] (d) {$d$} (0, 1) (0, 0) -- node [above] {$r$} (1, 0);
  }
  & $A = \pi r^2$ & $P = 2\pi r$ \\
  ...
  % triangle
  \tikz [baseline={(t.base)}] {
    \useasboundingbox (-1.2, -1.8) rectangle (1.2, 1.2);
  ...
  & $A = h\left(\frac{a+b}{2}\right)$ & (varies) \\
...
\DeclareFactoid{Psf}{% Parabolas (Standard Form)
$$
\begin{array}{|l|c|}
\hline \multicolumn{2}{|c|}{\text { Parabolas (Standard Form) }} \\
\hline \text { Equation } & y=a x^2+b x+c \\
\hline \text { Vertex } & \left(\frac{-b}{2 a}, f\left(\frac{-b}{2 a}\right)\right) \\
\hline \text { Axis of Symmetry } & x=\frac{-b}{2 a} \\
\hline \text { Opening Direction } & \begin{array}{c}
\text { up if } a>0 \\
\text { down if } a<0
\end{array} \\
\hline \text { y-intercept } & (0, c) \\
\hline
\end{array}
$$
}
...
\DeclareFactoid{sinusoidal graphs}{%
{\LARGE \textbf{\textcolor{Sepia}{What are midline, amplitude, and period?}}}\\
Midline, amplitude, and period are three features of sinusoidal graphs. \\\\
\begin{center}
\begin{tikzpicture}
\begin{axis}[axis lines=middle,xticklabels={},yticklabels={},xtick={-2*pi,-pi,0,pi,2*pi},ytick={-2,0,2},grid=major,ymin=-3,ymax=3,xmin=-2.5*pi,xmax=2.5*pi,width=10cm,height=6cm]
...
\draw[<->,Cyan,thick] (axis cs:-pi/2,-2.5)--(axis cs:3*pi/2,-2.5) node[midway,above,font=\footnotesize]{Period};
\end{axis}
\end{tikzpicture}
\end{center}
\textcolor{Maroon}{\underline{Midline}} is the horizontal line that passes exactly in the middle between the graph's maximum and minimum points.
\\\\
\textcolor{LimeGreen}{\underline{Amplitude}} is the vertical distance between the midline and one of the extremum points.
\\\\
\textcolor{Periwinkle}{\underline{Period}} is the distance between two consecutive maximum points, or two consecutive minimum points (these distances must be equal).
...
\DeclareFactoid{Trapezoidal Rule}{%
...
\DeclareFactoid{Path Integral}{%
$\underline{\textbf{Path integral:}}$ $\newline$\\
$\psi_t(y)=\int \psi_0(x) \int_x^y e^{i S / \hbar} D[x] \mathrm{d} x$
}
\DeclareFactoid{FTC1}{%
